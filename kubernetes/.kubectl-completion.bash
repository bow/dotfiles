...
__k_handle_reply()
{
    ...
    case $cur in
        -*)
            ...
            if [[ $cur == *=* ]]; then
                ...
                local index flag
                flag="${cur%=*}"
                __k_index_of_word "${flag}" "${flags_with_completion[@]}"
                COMPREPLY=()
                if [[ ${index} -ge 0 ]]; then
                    PREFIX=""
                    cur="${cur#*=}"
    ...
    local index
    __k_index_of_word "${prev}" "${flags_with_completion[@]}"
    if [[ ${index} -ge 0 ]]; then
        ${flags_completion[${index}]}
        return
    fi
...
__k_handle_filename_extension_flag()
{
    local ext="$1"
    _filedir "@(${ext})"
}

__k_handle_subdirs_in_dir_flag()
{
    local dir="$1"
...
__k_handle_flag()
{
    ...
    __k_debug "${FUNCNAME[0]}: looking for ${flagname}"
    if __k_contains_word "${flagname}" "${must_have_one_flag[@]}"; then
        must_have_one_flag=()
    fi

    # if you set a flag which only applies to this command, don't show subcommands
    if __k_contains_word "${flagname}" "${local_nonpersistent_flags[@]}"; then
      commands=()
    fi
    ...
    if __k_contains_word "${words[c]}" "${two_word_flags[@]}"; then
        c=$((c+1))
...
__k_handle_noun()
{
    ...
    if __k_contains_word "${words[c]}" "${must_have_one_noun[@]}"; then
        must_have_one_noun=()
    elif __k_contains_word "${words[c]}" "${noun_aliases[@]}"; then
        must_have_one_noun=()
    fi
...
__k_handle_command()
{
    ...
    if [[ -n ${last_command} ]]; then
        ...
        if [[ $c -eq 0 ]]; then
            next_command="_k_root_command"
        else
            next_command="_${words[c]//:/__}"
        fi
    fi
    c=$((c+1))
    __k_debug "${FUNCNAME[0]}: looking for ${next_command}"
...
# $2 is template string for k get (optional)
__k_parse_get()
{
    local template
...
__k_get_resource()
{
    if [[ ${#nouns[@]} -eq 0 ]]; then
      local k_out
      if k_out=$(k api-resources $(__k_override_flags) -o name --cached --request-timeout=5s --verbs=get 2>/dev/null); then
          COMPREPLY=( $( compgen -W "${k_out[*]}" -- "$cur" ) )
          return 0
      fi
      return 1
...
__k_get_resource_clusterrole()
{
    __k_parse_get "clusterrole"
}
...
__k_get_containers()
{
    ...
    __k_debug "${FUNCNAME} nouns are ${nouns[*]}"
...
__k_cp()
{
    ...
    case "$cur" in
        ...
        */*) # complete <namespace>/<pod>
            local template namespace k_out
            template="{{ range .items }}{{ .metadata.namespace }}/{{ .metadata.name }}: {{ end }}"
            namespace="${cur%%/*}"
            if k_out=( $(k get $(__k_override_flags) --namespace "${namespace}" -o template --template="${template}" pods 2>/dev/null) ); then
                COMPREPLY=( $(compgen -W "${k_out[*]}" -- "${cur}") )
            fi
            return
            ;;
        *) # complete namespaces, pods, and filedirs
            __k_parse_get "namespace" "{{ range .items  }}{{ .metadata.name }}/ {{ end }}"
            __k_parse_get "pod" "{{ range .items  }}{{ .metadata.name }}: {{ end }}"
            _filedir
            ;;
    esac
}
...
__custom_func() {
    case ${last_command} in
        ...
        k_apply_edit-last-applied | k_apply_view-last-applied)
            __k_get_resource
            return
            ;;
        k_logs)
            ...
            return
            ;;
        k_exec | k_port-forward | k_top_pod | k_attach)
            __k_get_resource_pod
            return
            ;;
        k_rolling-update)
        ...
        k_cordon | k_uncordon | k_drain | k_top_node)
            ...
            return
            ;;
        k_config_use-context | k_config_rename-context)
            __k_config_get_contexts
            return
            ;;
...
_k_annotate()
{
    ...
    flags_completion+=("__k_handle_filename_extension_flag json|yaml|yml")
    two_word_flags+=("-f")
...
_k_create_clusterrolebinding()
{
    ...
    flags_completion+=("__k_get_resource_clusterrole")
...
_k_create_priorityclass()
{
    last_command="k_create_priorityclass"
    commands=()
...
_k_rollout_restart()
{
    last_command="k_rollout_restart"
    commands=()
